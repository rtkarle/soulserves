...
if ($price_min  !== null)   $where .= " AND p.price >= "     . (float)$price_min;
if ($price_max  !== null)   $where .= " AND p.price <= "     . (float)$price_max;
if ($min_rating !== null)   $where .= " AND p.avg_rating >= " . (float)$min_rating;
...
/* ── Total count ── */
$cnt_q = $conn->query("SELECT COUNT(*) c FROM products p JOIN seller_stores s ON s.seller_email=p.seller_email WHERE $where");
$total_products = $cnt_q ? (int)$cnt_q->fetch_assoc()['c'] : 0;
$total_pages    = max(1, (int)ceil($total_products / $per_page));

/* ── Products ── */
$pq = $conn->query(
    "SELECT p.*, s.store_name, s.village, COALESCE(s.state, s.district, '') AS state
     FROM products p
...
/* ── Seller count ── */
$seller_count = (int)$conn->query("SELECT COUNT(*) c FROM seller_stores WHERE is_active=1")->fetch_assoc()['c'];
...
:root{
  ...
  --pink:#F72585;--bg:#F7FAF9;--card:#fff;
...
/* ── HEADER ── */
header{position:sticky;top:0;background:rgba(255,255,255,.95);backdrop-filter:blur(16px);box-shadow:0 2px 18px rgba(16,42,67,.07);z-index:200;border-bottom:1px solid rgba(226,235,233,.8)}
...
.search-bar input{flex:1;padding:11px 16px;border:1.5px solid var(--border);border-right:none;border-radius:10px 0 0 10px;font-size:14px;outline:none;background:#fafaf6;color:var(--text)}
.search-bar input:focus{border-color:var(--teal)}
.search-bar button{padding:0 18px;background:var(--grad);color:#fff;border:none;border-radius:0 10px 10px 0;cursor:pointer;font-size:16px;font-weight:700}
...
.cart-btn{position:relative;background:var(--grad) !important;color:#fff !important;border-radius:10px !important;padding:8px 16px !important}
...
.prod-card{background:var(--card);border-radius:var(--radius);overflow:hidden;box-shadow:var(--shadow);transition:.3s;cursor:pointer;border:1px solid rgba(16,42,67,.05);position:relative;display:flex;flex-direction:column}
...
.prod-rating{font-size:12px;color:var(--muted);margin-bottom:8px}
...
.add-cart-btn{flex:1;padding:9px;border:none;border-radius:10px;background:var(--grad);color:#fff;font-size:12px;font-weight:800;cursor:pointer;transition:.25s;box-shadow:0 4px 14px rgba(0,109,119,.15)}
.add-cart-btn:hover{transform:translateY(-1px);box-shadow:0 8px 22px rgba(46,139,87,.25)}
...
.view-btn{padding:9px 12px;border-radius:10px;border:1.5px solid var(--border);background:#fff;color:var(--muted);font-size:12px;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;transition:.2s;white-space:nowrap}
...
.page-btn{padding:8px 14px;border-radius:10px;border:1.5px solid var(--border);background:#fff;color:var(--muted);font-size:13px;font-weight:700;text-decoration:none;transition:.2s;cursor:pointer}
.page-btn:hover{border-color:var(--teal);color:var(--teal)}
.page-btn.active{background:var(--grad);color:#fff;border-color:transparent;box-shadow:0 4px 14px rgba(0,109,119,.25)}
...
.modal-box{background:#fff;border-radius:24px;padding:36px 32px;max-width:440px;width:100%;box-shadow:0 32px 80px rgba(0,0,0,.3);animation:modalIn .3s cubic-bezier(.22,1,.36,1);max-height:92vh;overflow-y:auto;position:relative}
...
.modal-title{font-size:22px;font-weight:900;color:var(--navy);text-align:center;margin-bottom:6px}
...
.mf-btn{width:100%;padding:13px;border:none;border-radius:10px;background:var(--grad);color:#fff;font-size:14px;font-weight:800;cursor:pointer;transition:.2s;margin-top:6px}
...
@media(max-width:768px){
  ...
  .view-btn{text-align:center;justify-content:center}
}
@media(max-width:480px){
  .nav{flex-wrap:wrap;height:auto;padding:10px 14px;gap:8px}
  .nav-logo img{height:28px}
  .search-bar{max-width:none;order:3;width:100%}
  ...
  <div class="nav">
    ...
    <div class="nav-actions">
      ...
      <?php else: ?>
        ...
        <button onclick="openGuestModal('register')" class="cart-btn">🛒 Cart</button>
      <?php endif; ?>
    </div>
  </div>
</header>
...
<div class="page">
  ...
  <div class="shop-hero">
    ...
    <div class="hero-stats">
      <div class="hs"><div class="hs-v"><?=$total_products?></div><div class="hs-l">Products</div></div>
      <div class="hs"><div class="hs-v"><?=$seller_count?></div><div class="hs-l">Artisans</div></div>
    </div>
  </div>
  ...
  <form method="GET" id="filterForm">
    <div class="filters-wrap">
      <div class="filter-row1">
        <div class="cat-btns">
          <a href="?<?=http_build_query(array_merge($_GET,['cat'=>'all','page'=>1]))?>" class="cat-btn <?=$cat==='all'?'active':''?>">All</a>
          <?php foreach($cats as $v=>$l): ?>
          <a href="?<?=http_build_query(array_merge($_GET,['cat'=>$v,'page'=>1]))?>" class="cat-btn <?=$cat===$v?'active':''?>"><?=$l?></a>
          <?php endforeach; ?>
        </div>
      ...
      <div class="filter-row2">
        <div class="filter-group">
          <label>💰 Price (₹)</label>
          <input type="number" name="pmin" class="price-input" placeholder="Min" value="<?=$price_min!==null?(int)$price_min:''?>" min="0" max="<?=$global_max?>">
          <span>—</span>
          <input type="number" name="pmax" class="price-input" placeholder="Max" value="<?=$price_max!==null?(int)$price_max:''?>" min="0" max="<?=$global_max?>">
        </div>
        <div class="filter-group">
          <label>⭐ Rating</label>
        ...
        <?php endif; ?>
        <span class="results-count"><?=$total_products?> result<?=$total_products!=1?'s':''?></span>
      </div>
    </div>
  </form>
  ...
  <div class="products-grid" id="productsGrid">
    ...
    ?>
    <div class="prod-card">
      <a href="product.php?id=<?=(int)$p['id']?>" style="display:block;text-decoration:none;color:inherit">
        <?php if($img): ?>
        <img src="<?=htmlspecialchars($img)?>" class="prod-img" alt="<?=htmlspecialchars($p['name'])?>" loading="lazy">
        <?php else: ?>
        <div class="prod-img-ph">🛍️</div>
        <?php endif; ?>
        <?php if($disc>0): ?>
        <div style="position:absolute;top:10px;left:10px;background:linear-gradient(135deg,#FF8A00,#F72585);color:#fff;font-size:10px;font-weight:800;padding:3px 9px;border-radius:20px"><?=$disc?>% off</div>
        <?php endif; ?>
      </a>
      <div class="prod-body">
        ...
        <div class="prod-actions">
          ...
          <a href="product.php?id=<?=(int)$p['id']?>" class="view-btn" onclick="event.stopPropagation()">👁️</a>
        </div>
      </div>
    </div>
    <?php endforeach; ?>
  ...
  <div class="pagination">
    <a href="<?=page_url($page-1)?>" class="page-btn <?=$page<=1?'disabled':''?>">← Prev</a>
    <?php
    $start = max(1, $page-2);
    $end   = min($total_pages, $page+2);
    if($start>1): ?><a href="<?=page_url(1)?>" class="page-btn">1</a><?php if($start>2): ?><span style="padding:8px 4px;color:var(--muted)">…</span><?php endif; endif;
    for($i=$start;$i<=$end;$i++): ?>
    <a href="<?=page_url($i)?>" class="page-btn <?=$i===$page?'active':''?>"><?=$i?></a>
    <?php endfor;
    if($end<$total_pages): ?><?php if($end<$total_pages-1): ?><span style="padding:8px 4px;color:var(--muted)">…</span><?php endif; ?><a href="<?=page_url($total_pages)?>" class="page-btn"><?=$total_pages?></a><?php endif; ?>
    <a href="<?=page_url($page+1)?>" class="page-btn <?=$page>=$total_pages?'disabled':''?>">Next →</a>
  </div>
  <div style="text-align:center;margin-top:10px;font-size:12px;color:var(--muted)">Page <?=$page?> of <?=$total_pages?> · <?=$total_products?> products</div>
  <?php endif; ?>
    ...
    <div class="prod-card">
      ...
      <div class="prod-body">
        ...
        <div class="prod-actions">
          ...
          <a href="product.php?id=<?=(int)$p['id']?>" class="view-btn">👁️</a>
        </div>
      </div>
    </div>
    <?php endforeach; ?>
...
<div class="modal-overlay" id="guestModal">
  <div class="modal-box">
    ...
    <div class="modal-form active" id="form-register">
      ...
      <div class="mf-group"><label class="mf-label">Full Name *</label><input class="mf-input" type="text" id="reg-name" placeholder="Your name" autocomplete="name" required></div>
      <div class="mf-group"><label class="mf-label">Email *</label><input class="mf-input" type="email" id="reg-email" placeholder="you@email.com" autocomplete="email" required></div>
      <div class="mf-group"><label class="mf-label">Password *</label><input class="mf-input" type="password" id="reg-pwd" placeholder="Min. 6 characters" autocomplete="new-password" required></div>
      <div class="mf-group"><label class="mf-label">Phone (optional)</label><input class="mf-input" type="tel" id="reg-phone" placeholder="+91 98765 43210" autocomplete="tel"></div>
      <button class="mf-btn" id="reg-btn" onclick="submitRegister()">Create Account & Continue Shopping →</button>
      <p style="text-align:center;font-size:12px;color:var(--muted);margin-top:12px;line-height:1.6">By registering you agree to our Terms of Service.</p>
    </div>
    <!-- Login -->
    <div class="modal-form" id="form-login">
      <div id="login-error" class="mf-error"></div>
      <div id="login-success" class="mf-success"></div>
      <div class="mf-group"><label class="mf-label">Email</label><input class="mf-input" type="email" id="login-email" placeholder="you@email.com" autocomplete="email"></div>
      <div class="mf-group"><label class="mf-label">Password</label><input class="mf-input" type="password" id="login-pwd" placeholder="Your password" autocomplete="current-password"></div>
...
const CSRF = '<?=$is_logged_in?csrf_token():''?>';
...
function submitRegister(){
  ...
  if(!name||!email||!pwd){errEl.textContent='Please fill all required fields.';errEl.style.display='block';return;}
  ...
  .then(r=>r.json()).then(d=>{
    ...
    if(d.ok){sucEl.textContent=d.message;sucEl.style.display='block';setTimeout(()=>location.reload(),1200);}
    else{errEl.textContent=d.message;errEl.style.display='block';}
  }).catch(()=>{btn.disabled=false;errEl.textContent='Network error. Try again.';errEl.style.display='block';});
}
